...
#'
#' @export
astar <- function(start, goal, density_mat, coeff,
  ...
  while (!is.null(datastructures::peek(open_set))) {
    ...
    for (neighbor in neighbors(crnt$data)) {
      ...
      tentative_gscore <- crnt$gscore + edge_distance(crnt$data, neigh_node$data, density_mat, coeff)
...
AStar <- R6::R6Class("AStar", list(
  ...
  run = function(start, goal) {
    astar(start, goal, self$density_mat, self$coeff,
